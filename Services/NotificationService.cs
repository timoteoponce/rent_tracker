...
public class NotificationService : INotificationService
{
    ...
    public async Task EnsurePendingPaymentsAsync()
    {
        ...
        foreach (var lease in allLeases)
        {
            foreach (var period in new HashSet<DateTimeOffset> { currentMonth, nextPeriod })
            {
                if (await HasAnyPaymentForPeriodAsync(lease.Id, period))
                    continue;
    ...
    private async Task<bool> HasAnyPaymentForPeriodAsync(Guid leaseId, DateTimeOffset forPeriod)
    {
        var payments = await _context.Payments
            .Where(p => p.LeaseId == leaseId)
            .ToListAsync();
