...
public class NotificationService : INotificationService
{
    ...
    /// <summary>
    /// Ensures active leases have a "Pending" Payment row for the expected rent,
    /// so property owners can see upcoming/expected payments without manual entry.
    /// Runs daily, independent of WhatsApp settings.
    /// Only covers the current period and the next upcoming due period.
    /// Idempotent: never inserts a payment for a period that already has one.
    /// </summary>
    public async Task EnsurePendingPaymentsAsync()
    {
        var today = DateTimeOffset.UtcNow;
        var currentMonth = new DateTimeOffset(today.Year, today.Month, 1, 0, 0, 0, TimeSpan.Zero);
        var nextPeriod = currentMonth.AddMonths(1);
        var periods = new[] { currentMonth, nextPeriod };

        var allLeases = await _context.Leases
            .Where(l => l.Status == LeaseStatus.Active)
            .ToListAsync();

        var activeLeaseIds = allLeases.Select(l => l.Id).ToHashSet();

        // One query: every (lease, year, month) that already has a payment of any status.
        var existingPayments = await _context.Payments
            .Where(p => activeLeaseIds.Contains(p.LeaseId))
            .Select(p => new { p.LeaseId, p.ForPeriod })
            .ToListAsync();

        var covered = existingPayments
            .Select(p => (p.LeaseId, p.ForPeriod.Year, p.ForPeriod.Month))
            .ToHashSet();
        ...
        foreach (var lease in allLeases)
        {
            ...
            {
                if (covered.Contains((lease.Id, period.Year, period.Month)))
                    continue;
                ...
                covered.Add((lease.Id, period.Year, period.Month));
            }
        }
        ...
        {
            // The (LeaseId, ForPeriod) filtered unique index rejected a row a concurrent
            // run already inserted. The placeholder exists either way, so this is safe to ignore.
        }
    }
